...
public static class GrpcWebBenchmarker
{
    ...
    public static async Task RunMenuAsync()
    {
        ...
        var webHandler = new GrpcWebHandler(GrpcWebMode.GrpcWeb, baseHandler);
        var handler = new LoggingHandler { InnerHandler = webHandler };
    ...
    private static async Task BenchmarkTextAsync(Text.TextClient client, Empty empty)
    {
        while (true)
        {
            ...
            Console.WriteLine($"gRPC-Web Text({key}): {bytes} bytes in {sw.ElapsedMilliseconds} ms\n");
        }
    }
